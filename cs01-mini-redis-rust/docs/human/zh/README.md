...
一个用 Rust 从零实现的 Redis 兼容子集,带 SvelteKit 浏览器监控控制台。目标是验证 ADSD 方法论在网络服务 + 持久化 + 前端领域是否仍然有效。
...
监控页:浏览器 dev mode 用 `cd web && pnpm dev` 打开 `http://localhost:5173`。
...
- [ADR-0001 栈选择](./adr-0001-stack-choice.md):tokio + Axum + hashbrown + SvelteKit/browser UI
...
✅ M1 backend MVP;✅ M2 frontend/control-plane MVP;✅ M3 Pub/Sub + AOF;✅ M4.1 critical fixes;✅ M4.2 doc sweep + release artifacts。Tauri desktop/sidecar 不是本项目需求,此前相关表述属于跨 session 串台,已撤回。
