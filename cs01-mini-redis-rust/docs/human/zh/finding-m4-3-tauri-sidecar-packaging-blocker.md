...
完整 bundle gate 又暴露了跨生态 Tauri minor mismatch:`@tauri-apps/api` 2.9.0 / `@tauri-apps/cli` 2.9.5 对上 Rust `tauri` 2.11.1 会被 bundler 拒绝。修复选择前滚 npm 到 2.11 line 并显式 pin:`@tauri-apps/api` 2.11.0、`@tauri-apps/cli` 2.11.0、`tauri` 2.11.1、`tauri-build` 2.6.1。随后默认 DMG target 已到达 packaging 但 Finder AppleScript layout timeout;gate 改验 `.app` bundle,DMG/signing/notarization 仍是 release engineering 风险。
