...
The M4.4 full-bundle gate first failed on an npm Tauri 2.9.x versus Rust `tauri` 2.11.1 minor-version mismatch; the fix explicitly pins `@tauri-apps/api`/`@tauri-apps/cli` to 2.11.0, `tauri` to 2.11.1, and `tauri-build` to 2.6.1 so lockfile regeneration cannot drift back. After that fix, the default DMG target still depended on Finder AppleScript layout and could time out in the build session, so the release-readiness gate now deterministically verifies `pnpm tauri build --bundles app`; that `.app` bundle gate passed at `31b52a1`, while DMG/signing/notarization remain separate future release-engineering work.
